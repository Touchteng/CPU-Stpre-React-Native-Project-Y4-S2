...
export const signUp = (email, password) => {
  return async (dispatch) => {
    const response = await fetch(
      'https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyBgiOjF6YZ1gcMvoqKEBgn-z_togPbsGtY',
      {
        method: 'POST',
        headers: {
...
export const signIn = (email, password) => {
  return async (dispatch) => {
    const response = await fetch(
      'https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyBgiOjF6YZ1gcMvoqKEBgn-z_togPbsGtY',
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
